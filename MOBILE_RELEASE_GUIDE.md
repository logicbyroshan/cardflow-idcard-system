...
- Java 17 JDK
- Gradle wrapper in `adarsh_native_app/android/`
- `release.keystore` file in `adarsh_native_app/android/app/`
...
cat adarsh_native_app/app.json | grep -E '"version"|"versionCode"'
...
**Output Location**:
```
adarsh_native_app/android/app/build/outputs/bundle/release/app-release.aab
```
...
cd ../..
mkdir -p tmp_verify
jar xf adarsh_native_app/android/app/build/outputs/bundle/release/app-release.aab META-INF/*.RSA META-INF/*.MF
...
```
Solution: Ensure adarsh_native_app/android/keystore.properties exists and paths are correct.
```
...
3. **Commit**:
   ```bash
   git add adarsh_native_app/app.json VERSION.txt
...
### Never Delete `adarsh_native_app/`
...
If deleted: `git checkout HEAD -- adarsh_native_app/`

### Backup Keystore
The `release.keystore` is critical. Keep these copies:
1. `adarsh_native_app/android/app/release.keystore` (primary)
2. `adarsh_native_app/android_backup/release.keystore` (backup)
