...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const dashboardTabCountBatchJobs = document.getElementById('dashboardTabCountBatchJobs');
    ...
    function formatBatchJobDuration(totalSeconds) {
    ...
    function formatBatchJobUpdatedAt() {
        const now = new Date();
        return `Updated ${now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`;
    }

    function batchJobStatusClass(status) {
        const normalized = String(status || '').trim().toLowerCase();
        if (!normalized) return '';
        return 'is-' + normalized;
    }

    function renderBatchJobProgressRows(tasks) {
        const esc = typeof escapeHtml === 'function'
            ? escapeHtml
            : (s) => String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\"/g, '&quot;');
        ...
        return tasks.map((task) => {
            ...
            const statusDisplay = esc(task.status_display || task.status || 'Unknown');
            const typeDisplay = esc(task.task_type_display || task.task_type || 'Task');
            const owner = esc(task.owner_name || 'You');
            const stage = esc(task.stage_label || statusDisplay);
            const progress = Number(task.progress) || 0;
            const total = Number(task.total) || 0;
            const progressPercentage = Math.max(0, Math.min(100, Number(task.progress_percentage) || 0));
            const elapsed = formatBatchJobDuration(task.elapsed_seconds);
            const eta = task.eta_seconds == null ? '-' : formatBatchJobDuration(task.eta_seconds);
            ...
            const cancelBtn = task.can_cancel
                ? `<button type="button" class="batch-job-action-btn" data-batch-job-cancel="${task.task_id}">Cancel</button>`
                : '';
            const downloadBtn = task.download_url
                ? `<button type="button" class="batch-job-action-btn" data-batch-job-download="${task.download_url}">Download</button>`
                : '';
            ...
            return `
                <div class="batch-job-item">
                    <div class="batch-job-row">
                        ...
                        <span class="batch-job-status-pill ${batchJobStatusClass(status)}">${statusDisplay}</span>
                    </div>
                    ...
                    <div class="batch-job-meta">
                        ...
                        <span>ETA: ${eta}</span>
                        <span>Created: ${esc(createdAt)}</span>
    ...
    function bindBatchJobProgressActions() {
        const listEl = document.getElementById('batchJobProgressList');
        if (!listEl || listEl.dataset.boundActions === '1') return;

        listEl.addEventListener('click', function(event) {
            const cancelBtn = event.target.closest('[data-batch-job-cancel]');
            if (cancelBtn) {
                event.preventDefault();
                const taskId = cancelBtn.getAttribute('data-batch-job-cancel');
                if (!taskId) return;
                cancelBtn.disabled = true;
                ApiClient.post(panelUrl('/api/task-cancel/' + taskId + '/'), {})
                    .then(function(data) {
                        if (data && data.success && typeof showToast === 'function') {
                            showToast(data.message || 'Task cancel requested', 'success');
                        } else if (typeof showToast === 'function') {
                            showToast((data && data.message) || 'Unable to cancel task', 'error');
                        }
                        loadBatchJobProgressCenter();
                    })
                    .catch(function() {
            ...
            if (downloadBtn) {
                ...
                window.location.href = panelUrl(downloadUrl);
            }
        });
    ...
    function loadBatchJobProgressCenter() {
        const listEl = document.getElementById('batchJobProgressList');
        if (!listEl) return;
        ...
        ApiClient.get(panelUrl('/api/task-progress-center/?limit=8'))
            ...
            .catch((error) => {
                console.error('Error loading batch job progress center:', error);
                setDashboardTabCount(dashboardTabCountBatchJobs, 0);
