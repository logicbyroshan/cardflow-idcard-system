...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const passwordOptionDropdown = document.getElementById('staffPasswordOptionDropdown');
    const passwordOptionInput = document.getElementById('staff-password-option');
    const customPasswordGroup = document.getElementById('staffCustomPasswordGroup');
    ...
    function setPasswordOption(val) {
        ...
        if (passwordOptionDropdown) {
            const toggle = passwordOptionDropdown.querySelector('.dropdown-toggle span');
            const options = passwordOptionDropdown.querySelectorAll('.dropdown-option');
            options.forEach(o => o.classList.remove('selected'));
            const match = passwordOptionDropdown.querySelector(`.dropdown-option[data-value="${val}"]`);
            if (match) {
                match.classList.add('selected');
                if (toggle) toggle.textContent = match.textContent;
            }
    ...
    if (passwordOptionDropdown && passwordOptionInput) {
        const toggleBtn = passwordOptionDropdown.querySelector('.dropdown-toggle');
        const options = passwordOptionDropdown.querySelectorAll('.dropdown-option');
        if (toggleBtn) {
            toggleBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                document.querySelectorAll('.custom-dropdown.open').forEach(d => { if (d !== passwordOptionDropdown) d.classList.remove('open'); });
    ...
    function openDrawer(mode = 'add', staffData = null) {
        ...
        setPasswordOption('phone'); // Reset password option
        ...
        if (mode === 'add') {
            ...
            const pwRow = document.getElementById('staffPasswordOptionRow');
            if (pwRow) pwRow.style.display = '';
            
            // Hide temp password button in add mode
            const tempPwBtn = document.getElementById('tempPasswordStaffBtn');
    ...
    if (staffForm) {
        staffForm.addEventListener('submit', async (e) => {
            ...
            const pwOpt = document.getElementById('staff-password-option');
            if (pwOpt && pwOpt.value === 'custom') {
                const pwVal = document.getElementById('staff-password');
    ...
    let tempPwVerificationCode = '';
    let tempPwTargetType = 'staff';
    let tempPwTargetId = null;
    let tempPwTargetName = '';

    window.openTempPasswordModal = function(type) {
        tempPwTargetType = type || 'staff';
        tempPwTargetId = selectedStaffId;
    ...
    window.verifyTempPwCode = function() {
        const input = document.getElementById('tempPwCodeInput').value.trim();
        const errEl = document.getElementById('tempPwCodeError');
    ...
    window.toggleTempPwVisibility = function() {
        const pwInput = document.getElementById('tempPwNewPassword');
        const eyeIcon = document.getElementById('tempPwEyeIcon');
    ...
    window.saveTempPassword = async function() {
        const password = document.getElementById('tempPwNewPassword').value;
        const errEl = document.getElementById('tempPwError');
        ...
        const saveBtn = document.getElementById('tempPwSaveBtn');
        saveBtn.disabled = true;
        saveBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Saving...';

        try {
            const url = `/panel/api/staff/${tempPwTargetId}/set-temp-password/`;
            const result = await ApiClient.post(url, { password });
            if (result.success) {
