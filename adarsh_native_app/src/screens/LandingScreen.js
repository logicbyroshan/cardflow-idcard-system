...
export default function LandingScreen({ navigation }) {
  ...
  const loadLandingData = useCallback(async () => {
    try {
      ...
      throw new Error(res.message || 'Failed to load content. Please try again.');
    } catch (e) {
      console.error(e);
      throw e;
    }
  ...
  const { data: landingData = null, loading, error, refresh } = useRefreshableResource(loadLandingData, { initialData: null });
        ...
        <View style={s.root}>
          <Animated.View style={[s.header, { paddingTop: insets.top + 8, backgroundColor: headerBg, borderBottomWidth: headerShadow, borderBottomColor: '#eee' }]}>
            <View style={s.headerInner}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate(isAuthenticated ? 'Home' : 'Login')} style={s.loginBtn}>
          ...
          >
            ...
            <View style={s.quickCatRow}>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.quickCatScroll}>
                {landingData?.categories?.map(cat => (
                  <TouchableOpacity key={cat.id} onPress={() => navigation.navigate('ProductCategoryDetail', { category: cat })} style={s.quickCatChip}>
            ...
            <View style={s.section}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity style={s.featuredProdCard} onPress={() => navigation.navigate('ProductDetail', { product: item })}>
...
const s = StyleSheet.create({
  ...
  loginBtnGrad: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 10 },
  ...
  heroDots: { position: 'absolute', bottom: 20, left: 24, flexDirection: 'row' },
  ...
  quickCatChip: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f1f5f9', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20 },
  ...
  shelfMoreCard: { width: 100, height: 180, alignItems: 'center', justifyContent: 'center' },
  ...
  benefitsRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 16 },
  ...
  retryBtnGrad: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16 },
