...
# Higher minimum quality to preserve visual fidelity
_MIN_QUALITY = 60  # Changed from 5 to 60 - prevents severe quality loss
_MAX_QUALITY = 98  # Changed from 95 to 98 - allows higher quality output

# Resize scale factors - more conservative to preserve quality
_RESIZE_SCALES = (0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60, 0.55, 0.50)
...
def _compress_image(
    ...
    with Image.open(img_path) as im:
        ...
        if best_buf is None:
            # Even minimum quality exceeds target
            # Try a slightly lower quality before resorting to resize
            for fallback_quality in range(_MIN_QUALITY - 5, max(30, _MIN_QUALITY - 20), -5):
                if fallback_quality < 30:
                    break
                best_buf = _encode_jpeg(im, fallback_quality, exif_data)
                if len(best_buf) <= target_bytes:
                    break
            
            # If still too large, resort to resizing
            if best_buf is None or len(best_buf) > target_bytes:
...
def _encode_jpeg(im: Image.Image, quality: int, exif: bytes | None) -> bytes:
    ...
    Uses smart subsampling based on quality level:
    - Quality >= 90: 4:4:4 (no subsampling) for maximum quality
    - Quality >= 80: 4:2:2 (moderate subsampling) for good quality
    - Quality < 80: 4:2:0 (standard subsampling) for smaller files
    """
    buf = BytesIO()
    save_kwargs = {
        "format": "JPEG",
    ...
    # Smart subsampling based on quality
    if quality >= 90:
        save_kwargs["subsampling"] = 0  # 4:4:4 - no subsampling
    elif quality >= 80:
        save_kwargs["subsampling"] = 1  # 4:2:2 - moderate
    else:
        save_kwargs["subsampling"] = 2  # 4:2:0 - standard
...
def _compress_with_resize(
    ...
    it fits in *target_bytes* at reasonable JPEG quality.

    Uses more conservative resize steps to preserve quality.
    Returns the JPEG bytes or None if even 50% dimensions can't fit.
    """
    w, h = im.size
