...
# ── Global Stop Signal ───────────────────────────────────────────────────
# Used to abort long-running batch operations.
_STOP_SIGNAL = False

def set_stop_signal(value: bool) -> None:
    """Set the engine-wide stop signal."""
    global _STOP_SIGNAL
    _STOP_SIGNAL = value
    if value:
        logger.warning("Engine STOP signal received. Aborting current task...")
...
def process_single_image(
    ...
    if is_stopped():
        return {"status": "failed", "reason": "Task cancelled by user", "failure_code": "CANCELLED"}
    ...
    def process_batch(
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            ...
            for idx, future in enumerate(as_completed(future_to_path), start=1):
                if is_stopped():
                    logger.warning("Batch processing aborted via stop signal.")
                    break
