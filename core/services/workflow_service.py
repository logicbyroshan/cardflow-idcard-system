...
class ReprintWorkflowService:
    ...
    Usage:
        result = ReprintWorkflowService.transition(reprint_request, 'confirmed', user)
        result = ReprintWorkflowService.bulk_transition(table, rr_ids, 'confirmed', user)
    """
    ...
    def bulk_transition(
        ...
        Only transitions requests whose current status is a valid source
        for the target. E.g. only status='requested' can become 'confirmed'.
        """
        if target_status not in cls.VALID_STATUSES:
            return ServiceResult(success=False, message=f'Invalid reprint status: {target_status}')

        # Determine valid source statuses
        valid_from = [s for s, targets in cls.ALLOWED_TRANSITIONS.items() if target_status in targets]
        if not valid_from:
    ...
    def create_requests(
        ...
        Returns counts of created and skipped requests.
        """
        if not card_ids:
