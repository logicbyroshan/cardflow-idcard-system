...
class MobileAppBaseTestCase(TestCase):
	...
	def _collect_mobile_cards_api_ids(self, table_id, *, status='pending', params=None, user_agent=None, per_page=50, max_pages=20):
		...
		while True:
			response = self.client.get(
				f'/api/mobile/table/{table_id}/cards/',
				data=query_params,
				**request_headers,
			)
	...
	def test_mobile_api_returns_401_without_mobile_auth_checkpoint(self):
		self.client.login(username='mob-super@test.com', password='pass1234')
		response = self.client.get('/api/mobile/server-info/')
...
class MobileAppShellApiTests(MobileAppBaseTestCase):
	def test_mobile_shell_config_returns_policy_payload(self):
		...
		response = self.client.get('/api/mobile/mobile-shell/config/?app_build=1')
	...
	def test_mobile_shell_config_uses_explicit_update_url_when_configured(self):
		self._login_mobile_client()
		response = self.client.get('/api/mobile/mobile-shell/config/?app_build=10')
	...
	def test_mobile_shell_register_upserts_device(self):
		...
		response = self.client.post(
			'/api/mobile/mobile-shell/device/register/',
	...
	def test_mobile_shell_ping_creates_record_when_missing(self):
		...
		response = self.client.post(
			'/api/mobile/mobile-shell/device/ping/',
			data=json.dumps(payload),
			content_type='application/json',
		)
	...
	def test_mobile_shell_config_force_update_overrides_build_threshold(self):
		self._login_mobile_client()
		response = self.client.get('/api/mobile/mobile-shell/config/?app_build=9999')
		self.assertEqual(response.status_code, 200)
		self.assertTrue(response.json()['data']['update_required'])

	def test_mobile_shell_summary_requires_elevated_role(self):
		self._login_mobile_client()
		response = self.client.get('/api/mobile/mobile-shell/device/summary/')
...
class MobileAppCardApiTests(MobileAppBaseTestCase):
	...
	def test_filtered_select_all_ids_match_bulk_verify_with_expected_skips(self):
		...
		ids_response = self.client.get(
			f'/api/mobile/table/{table.id}/cards/all-ids/?status=pending&class=2&section=C'
		)
		...
		bulk_response = self.client.post(
			f'/api/mobile/table/{table.id}/bulk-status/',
	...
	def test_card_add_requires_add_permission_for_client_role(self):
		self._login_mobile_client()
		response = self.client.post(
			f'/api/mobile/table/{self.table.id}/card/add/',
	...
	def test_card_update_requires_edit_permission_for_client_role(self):
		self._login_mobile_client()
		response = self.client.post(
			f'/api/mobile/table/{self.table.id}/card/{self.card.id}/update/',
	...
	def test_card_delete_requires_delete_permission_for_client_role(self):
		self._login_mobile_client()
		response = self.client.post(
			f'/api/mobile/card/{self.card.id}/delete/',
	...
	def test_table_update_fields_rejects_too_many_fields(self):
		...
		response = self.client.post(
			f'/api/mobile/table/{self.table.id}/update-fields/',
	...
	def test_upload_photo_rejects_invalid_card_id(self, _mock_validate):
		...
		response = self.client.post(
			f'/api/mobile/table/{self.table.id}/upload-photo/',
	...
	def test_client_staff_update_card_blocked_outside_row_scope(self):
		...
		self._login_mobile_client_staff()
		response = self.client.post(
			f'/api/mobile/table/{self.table.id}/card/{out_of_scope_card.id}/update/',
	...
	def test_client_staff_delete_card_blocked_outside_row_scope(self):
		...
		self._login_mobile_client_staff()
		response = self.client.post(
			f'/api/mobile/card/{out_of_scope_card.id}/delete/',
			data=json.dumps({'permanent': False}),
			content_type='application/json',
		)
	...
	def test_mobile_card_detail_requires_card_info_permission(self):
		...
		self._login_mobile_client()
		response = self.client.get(f'/api/mobile/card/{self.card.id}/detail/')
...
class MobileAppManagementApiTests(MobileAppBaseTestCase):
	def test_mobile_impersonation_users_requires_pro_user(self):
		self._login_mobile_client()
		response = self.client.get('/api/mobile/impersonate/users/')
		self.assertEqual(response.status_code, 403)
		self.assertFalse(response.json()['success'])
	...
	def test_server_info_requires_super_admin(self):
		self._login_mobile_client()
		denied = self.client.get('/api/mobile/server-info/')
		self.assertEqual(denied.status_code, 403)

		self._login_mobile_super_admin()
		allowed = self.client.get('/api/mobile/server-info/')
		self.assertEqual(allowed.status_code, 200)
		self.assertTrue(allowed.json()['success'])

	def test_client_toggle_forbidden_for_non_super_admin(self):
		self._login_mobile_client()
		response = self.client.post(f'/api/mobile/client/{self.client_profile.id}/toggle/')
		self.assertEqual(response.status_code, 403)
	...
	def test_client_delete_stays_super_admin_only_for_admin_staff_with_manage_permission(self):
		self._login_mobile_admin_staff_manager()
		response = self.client.post(f'/api/mobile/client/{self.client_profile.id}/delete/')
		self.assertEqual(response.status_code, 403)

	def test_client_tables_requires_admin_role(self):
		self._login_mobile_client()
		denied = self.client.get(f'/api/mobile/client/{self.client_profile.id}/tables/')
		self.assertEqual(denied.status_code, 403)

		self._login_mobile_super_admin()
		allowed = self.client.get(f'/api/mobile/client/{self.client_profile.id}/tables/')
		self.assertEqual(allowed.status_code, 200)
		self.assertTrue(allowed.json()['success'])

	def test_search_short_query_returns_empty_success_payload(self):
		self._login_mobile_super_admin()
		response = self.client.get('/api/mobile/search/?q=a')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
	...
	def test_search_matches_dynamic_field_like_desktop_global_search(self):
		...
		response = self.client.get('/api/mobile/search/?q=MOB-GLOBAL-SEARCH-777')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
	...
	def test_mobile_global_search_api_respects_client_staff_row_scope(self):
		...
		self._login_mobile_client_staff()
		response = self.client.get(f'/api/mobile/search/?q=Scoped Search Student&filter=name&table_id={self.table.id}')
	...
	def test_mobile_global_search_api_hides_statuses_without_list_permission(self):
		...
		self._login_mobile_client_staff()
		response = self.client.get(f'/api/mobile/search/?q=Status Scope Student&filter=name&table_id={self.table.id}')
	...
	def test_mobile_list_api_search_matches_dynamic_table_field(self):
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=MOB-LIST-SEARCH-919')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
	...
	def test_mobile_list_api_requires_explicit_valid_status(self):
		...
		missing_status = self.client.get(f'/api/mobile/table/{self.table.id}/cards/')
		self.assertEqual(missing_status.status_code, 400)
		self.assertEqual(missing_status.json().get('message'), 'status is required')

		invalid_status = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=invalid-status')
	...
	def test_mobile_list_api_pending_scope_excludes_approved_cards(self):
		...
		response = self.client.get(
			f'/api/mobile/table/{self.table.id}/cards/?status=pending&search={search_token}'
		)
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
	...
	def test_mobile_list_api_honors_class_and_section_filters(self):
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&class=10')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		ids = [item['id'] for item in payload['data']['cards']]
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&class=10&section=A')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		ids = [item['id'] for item in payload['data']['cards']]
	...
	def test_mobile_list_api_honors_name_sort_query(self):
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=Sort Name&sort=name-asc')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=Sort Name&sort=name-desc')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		names = [str(item.get('name') or '').strip() for item in payload['data']['cards']]
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=Sort Name&sort=name-desc&cursor=999999')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		names = [str(item.get('name') or '').strip() for item in payload['data']['cards']]
	...
	def test_mobile_list_api_honors_emp_name_sort_query(self):
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=Sort Emp&sort=name-asc')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		names = [str(item.get('name') or '').strip() for item in payload['data']['cards']]
	...
	def test_mobile_list_api_honors_search_and_photo_filters_for_full_dataset(self):
		...
		response = self.client.get(
			f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=Server Search Alpha&photo=with'
		)
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		...
		response = self.client.get(
			f'/api/mobile/table/{self.table.id}/cards/?status=pending&search=Server Search Alpha&photo=without'
		)
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
	...
	def test_mobile_filter_options_api_uses_full_dataset_not_loaded_rows_only(self):
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/filter-options/?status=pending')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
	...
	def test_mobile_list_initial_batch_and_load_more_use_same_page_size(self):
		...
		api_page_two = self.client.get(f'/api/mobile/table/{table.id}/cards/?status=pending&page=2&per_page=50')
	...
	def test_mobile_all_ids_api_honors_search_and_photo_filters(self):
		...
		response = self.client.get(
			f'/api/mobile/table/{self.table.id}/cards/all-ids/?status=pending&search=Select Scope&photo=with'
		)
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
	...
	def test_mobile_list_api_download_date_filter_uses_downloaded_at(self):
		...
		response = self.client.get(f'/api/mobile/table/{self.table.id}/cards/?status=download&from={from_date}')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		ids = [item['id'] for item in payload['data']['cards']]
	...
	def test_mobile_global_search_supports_filter_and_table_scope(self):
		...
		response = self.client.get(f'/api/mobile/search/?q=Scoped Person&filter=name&table_id={self.table.id}')
		self.assertEqual(response.status_code, 200)
		payload = response.json()
		self.assertTrue(payload['success'])
		ids = [item['id'] for item in payload['data']['results']]
	...
	def test_client_create_rejects_invalid_json_before_service_call(self, mock_create):
		self._login_mobile_super_admin()
		response = self.client.post('/api/mobile/client/create/', data='bad-json', content_type='application/json')
		self.assertEqual(response.status_code, 400)
	...
	def test_staff_api_list_requires_manage_client_permission_for_client_role(self):
		self._login_mobile_client()
		denied = self.client.get('/api/mobile/staff/')
		self.assertEqual(denied.status_code, 403)
		self.assertFalse(denied.json()['success'])

		self.client_profile.perm_idcard_client_list = True
		self.client_profile.save(update_fields=['perm_idcard_client_list'])
		allowed = self.client.get('/api/mobile/staff/')
		self.assertEqual(allowed.status_code, 200)
		self.assertTrue(allowed.json()['success'])
	...
	def test_website_upload_requires_website_edit_permission(self):
		self._login_mobile_client()
		response = self.client.post('/api/mobile/website/portfolio/upload/', data={})
		self.assertEqual(response.status_code, 403)
	...
	def test_website_upload_returns_partial_success_for_mixed_files(self, mock_create):
		...
		response = self.client.post(
			'/api/mobile/website/portfolio/upload/',
...
class MobileAppCoverageGapRegressionTests(MobileAppBaseTestCase):
	...
	def test_api_staff_toggle_and_delete_work_for_super_admin(self):
		...
		self._login_mobile_super_admin()
		toggle_response = self.client.post(f'/api/mobile/staff/{managed_staff.id}/toggle/')
		self.assertEqual(toggle_response.status_code, 200)
		managed_user.refresh_from_db()
		self.assertFalse(managed_user.is_active)

		delete_response = self.client.post(f'/api/mobile/staff/{managed_staff.id}/delete/')
	...
	def test_api_client_detail_for_super_admin_returns_service_payload(self, mock_get):
		...
		self._login_mobile_super_admin()
		response = self.client.get(f'/api/mobile/client/{self.client_profile.id}/')
		self.assertEqual(response.status_code, 200)
		self.assertTrue(response.json().get('success'))
	...
	def test_api_client_detail_for_admin_staff_manager_denies_out_of_scope_client(self, mock_get):
		...
		self._login_mobile_admin_staff_manager()
		response = self.client.get(f'/api/mobile/client/{other_client.id}/')
		self.assertEqual(response.status_code, 403)
		self.assertFalse(response.json().get('success'))
	...
	def test_api_client_update_for_super_admin_returns_service_payload(self, mock_update):
		...
		self._login_mobile_super_admin()
		response = self.client.post(
			f'/api/mobile/client/{self.client_profile.id}/update/',
	...
	def test_api_portfolio_category_items_requires_view_permission_and_returns_items(self):
		...
		self._login_mobile_client()
		denied = self.client.get(f'/api/mobile/website/portfolio/category/{category.id}/items/')
		self.assertEqual(denied.status_code, 403)

		self._login_mobile_super_admin()
		allowed = self.client.get(f'/api/mobile/website/portfolio/category/{category.id}/items/?limit=5')
		self.assertEqual(allowed.status_code, 200)
...
class MobileAppPhase4DeviceBridgeContractTests(TestCase):
	...
	def test_mobile_bridge_uses_critical_queue_and_push_refresh_hooks(self):
		...
		self.assertIn("enqueueCriticalJson('/api/mobile/mobile-shell/device/ping/'", content)
...
class MobileAppPhase5OfflineCachingContractTests(TestCase):
	...
	def test_service_worker_defines_route_policy_buckets(self):
		...
		self.assertIn('const ONLINE_REQUIRED_PREFIXES = ', content)
		self.assertIn('if (url.pathname.indexOf(\'/api/mobile/\') === 0)', content)
...
class MobileAppProfileUpdateFlowContractTests(TestCase):
	...
	def test_profile_template_shows_update_status_card_and_runtime_check(self):
		...
		self.assertIn("fetch('/api/mobile/mobile-shell/config/'", content)
...
class MobileAppPhase1SmokeAndVisualTests(MobileAppBaseTestCase):
	...
	def test_phase1_auth_login_api_smoke_success(self, mock_authenticate, _mock_log_login):
		...
		response = self._post_json('/api/mobile/auth/login/', {
	...
	def test_phase1_api_routes_smoke_matrix(self):
		...
		api_cases = [
			...
			{'method': 'post', 'url': f'/api/mobile/card/{self.card.id}/status/', 'payload': {'status': 'verified'}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/card/{temp_card_for_delete.id}/delete/', 'allowed': (200, 400, 403)},
			{'method': 'get', 'url': f'/api/mobile/table/{self.table.id}/cards/?status=pending', 'allowed': (200, 403)},
			{'method': 'post', 'url': f'/api/mobile/table/{self.table.id}/bulk-status/', 'payload': {'ids': [self.card.id], 'status': 'verified'}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/table/{self.table.id}/upload-photo/', 'payload': {'card_id': self.card.id, 'field_name': 'PHOTO'}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/table/{self.table.id}/card/add/', 'payload': {'field_data': {'NAME': 'Phase1 Add', 'ROLL NO': '501'}}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/table/{self.table.id}/card/{self.card.id}/update/', 'payload': {'field_data': {'NAME': 'Phase1 Update'}}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/table/{self.table.id}/update-fields/', 'payload': {'fields': ['NAME', 'ROLL NO']}, 'allowed': (200, 400, 403)},
			{'method': 'get', 'url': '/api/mobile/staff/', 'allowed': (200, 403)},
			{'method': 'post', 'url': '/api/mobile/staff/create/', 'payload': {'first_name': 'Phase', 'last_name': 'Staff', 'email': 'phase1-staff-create@test.com'}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/staff/{managed_staff.id}/update/', 'payload': {'first_name': 'Phase1', 'last_name': 'Updated'}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/staff/{managed_staff.id}/toggle/', 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/staff/{managed_staff.id}/delete/', 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': '/api/mobile/profile/update/', 'payload': {'name': 'Phase One User'}, 'allowed': (200, 400, 403)},
			{'method': 'get', 'url': f'/api/mobile/search/?q=Student&filter=name&table_id={self.table.id}', 'allowed': (200, 400, 403)},
			{'method': 'get', 'url': '/api/mobile/server-info/', 'allowed': (200, 403)},
			{'method': 'get', 'url': '/api/mobile/mobile-shell/config/', 'allowed': (200, 403)},
			{'method': 'post', 'url': '/api/mobile/mobile-shell/device/register/', 'payload': {'platform': 'android', 'installation_id': 'instphase1abc12345', 'app_build': 1, 'app_version': '1.0.0', 'device_model': 'Pixel', 'os_version': '14', 'device_language': 'en'}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': '/api/mobile/mobile-shell/device/ping/', 'payload': {'installation_id': 'instphase1abc12345', 'app_build': 1, 'app_version': '1.0.0'}, 'allowed': (200, 400, 403)},
			{'method': 'get', 'url': '/api/mobile/mobile-shell/device/summary/', 'allowed': (200, 403)},
			{'method': 'get', 'url': '/api/mobile/impersonate/users/', 'allowed': (200, 403)},
			{'method': 'post', 'url': '/api/mobile/impersonate/start/', 'payload': {'target_user_id': self.client_user.id}, 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': '/api/mobile/impersonate/stop/', 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': '/api/mobile/client/create/', 'payload': {'name': 'Phase1 Created Client', 'email': 'phase1-created-client@test.com'}, 'allowed': (200, 400, 403)},
			{'method': 'get', 'url': f'/api/mobile/client/{managed_client.id}/', 'allowed': (200, 400, 403)},
			{'method': 'post', 'url': f'/api/mobile/client/{managed_client.id}/update/', 'payload': {'name': 'Phase1 Client Updated'}, 'allowed': (200, 400, 403)},
